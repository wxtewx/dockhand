...
<script lang="ts">
	...
	async function stopBackup() {
		...
		try {
			...
			logModalLogs = [...logModalLogs, '⚠ 用户已取消备份'];
			logModalStatus = 'error';
			logModalError = '已取消';
		} catch { toast.error('停止备份失败'); }
	}
			...
			{#snippet cell(column, config, rowState)}
				...
				{:else if column.id === 'type'}
					<div class="flex justify-center">
						{#if config.type === 'container'}
							<Tooltip.Root><Tooltip.Trigger><Box class="w-3 h-3 text-muted-foreground" /></Tooltip.Trigger><Tooltip.Content class="whitespace-nowrap">容器备份</Tooltip.Content></Tooltip.Root>
						{:else}
							<Tooltip.Root><Tooltip.Trigger><Layers class="w-3 h-3 text-muted-foreground" /></Tooltip.Trigger><Tooltip.Content class="whitespace-nowrap">堆栈备份</Tooltip.Content></Tooltip.Root>
						{/if}
					</div>
				...
				{:else if column.id === 'lastBackup'}
					{#if config.lastBackupAt}
						<span class="text-xs">{formatTimestamp(config.lastBackupAt)}</span>
					{:else}
				...
				{:else if column.id === 'status'}
					<div class="flex items-center justify-center">
						{#if config.isOrphan}
							<Tooltip.Root><Tooltip.Trigger><AlertCircle class="w-3 h-3 text-amber-500" /></Tooltip.Trigger><Tooltip.Content class="whitespace-nowrap">不存在备份计划，但仓库内存在快照</Tooltip.Content></Tooltip.Root>
						{:else if runningBackup === config.id}
							<Tooltip.Root><Tooltip.Trigger><Loader2 class="w-3 h-3 text-primary animate-spin" /></Tooltip.Trigger><Tooltip.Content class="whitespace-nowrap">备份正在执行</Tooltip.Content></Tooltip.Root>
						{:else if config.lastBackupStatus === 'success'}
							<Tooltip.Root><Tooltip.Trigger><CheckCircle class="w-3 h-3 text-green-500" /></Tooltip.Trigger><Tooltip.Content class="whitespace-nowrap">上次备份成功</Tooltip.Content></Tooltip.Root>
						{:else if config.lastBackupStatus === 'failed'}
							<Tooltip.Root><Tooltip.Trigger><XCircle class="w-3 h-3 text-destructive" /></Tooltip.Trigger><Tooltip.Content class="whitespace-nowrap">上次备份失败</Tooltip.Content></Tooltip.Root>
						{:else}
							<Tooltip.Root><Tooltip.Trigger><AlertCircle class="w-3 h-3 text-muted-foreground" /></Tooltip.Trigger><Tooltip.Content class="whitespace-nowrap">尚未执行过备份</Tooltip.Content></Tooltip.Root>
						{/if}
					</div>
								...
								<tbody>
									{#each snapshots.slice(0, snapshotLimits.get(key) || SNAPSHOT_PAGE_SIZE) as snapshot}
										...
										<tr class="border-b last:border-0 hover:bg-muted/30 text-xs {isDiffPending ? 'bg-primary/10' : ''}">
											...
											<td class="py-1.5" style="padding-left:8px">{formatTimestamp(snapshot.time)}</td>
