	...
	async function loadSnapshots(config: BackupConfig) {
		...
		try {
			...
			if (!snapError) {
				...
				newMap.set(key, []);
				toast.error(cleanErrorMessage(snapError || '加载快照失败'));
			}
			snapshotsMap = newMap;
	...
	async function deleteConfig(config: BackupConfig) {
		...
		try {
			...
			if (res.ok) {
				...
				toast.success(`已移除 ${config.targetName} 的备份配置${withSnaps ? ` (已删除 ${n} 个快照)` : ''}`);
				fetchData();
			} else {
				const data = await res.json().catch(() => ({}));
...
<div class="flex-1 min-h-0 flex flex-col gap-3 overflow-hidden">
	<div class="shrink-0 flex flex-wrap justify-between items-center gap-3 min-h-8">
		<PageHeader title="备份管理" icon={Archive} count={filteredConfigs.length}>
			{#if loading}
		...
		<div class="flex flex-wrap items-center gap-2">
			<SearchInput bind:value={searchQuery} placeholder="筛选备份..." class="h-8 w-48 text-sm" />
					...
					<div class="flex items-center justify-end gap-1" onclick={(e) => e.stopPropagation()}>
						{#if !config.isOrphan}
							...
							<ConfirmPopover
								...
								title={deleteConfigSnapshots ? '快照也将被删除。' : '将保留现有快照。'}
								position="left"
									...
									{#each snapshots.slice(0, snapshotLimits.get(key) || SNAPSHOT_PAGE_SIZE) as snapshot}
										...
										<tr class="border-b last:border-0 hover:bg-muted/30 text-xs {isDiffPending ? 'bg-primary/10' : ''} {selectedSnapshots.has(snapshot.id) ? 'bg-primary/5' : ''}">
											<td class="py-1.5" style="padding-left:8px">
												<Checkbox checked={selectedSnapshots.has(snapshot.id)} onCheckedChange={() => toggleSnapshotSel(snapshot.id)} aria-label="选中快照 {snapshot.shortId}" />
											</td>
