			...
			if (response.ok) {
				...
				if (Array.isArray(data)) {
					...
					if (data.notSupported && results.length === 0) {
						errorMessage = data.message || '该仓库不支持检索全部镜像。';
					}
				}
			} else {
				const data = await response.json();
			...
			while (idx < queue.length) {
				...
				try {
					...
					setTagEnrich(imageName, tag.name, {
						...
						enrichReason: hasData ? undefined : (info?.reason || '无法获取')
					});
				} catch {
					setTagEnrich(imageName, tag.name, { enrichState: 'unavailable', enrichReason: '请求失败' });
				}
			}
		};
	...
	{:else if results.length > 0}
		...
		{#if browseMode}
			<div class="shrink-0 flex items-center gap-2 text-sm">
				<SearchInput bind:value={browseFilter} placeholder="筛选结果..." class="h-8 text-xs" containerClass="flex-1 max-w-xs" />
