...
<script lang="ts">
	...
	async function handleLoadTar(e: Event) {
		...
		loadingImage = true;
		const t = toast.loading(`正在加载 ${file.name}...`);
		try {
			...
			if (data?.success) {
				toast.success(data.loaded || `已从 ${file.name} 加载镜像`, { id: t });
				await fetchImages();
			} else {
				toast.error(data?.error || '镜像加载失败', { id: t });
			}
		} catch (err) {
			toast.error(err instanceof Error ? err.message : '镜像加载失败', { id: t });
		} finally {
			loadingImage = false;
		}
	}
...
<div class="flex-1 min-h-0 flex flex-col gap-3 overflow-hidden">
	<div class="shrink-0 flex flex-wrap justify-between items-center gap-3 min-h-8">
		...
		<div class="flex flex-wrap items-center gap-2">
			<SearchInput bind:value={searchQuery} placeholder="搜索镜像..." class="h-8 w-48 text-sm" />
		...
		<div class="flex flex-wrap items-center gap-2 w-full">
			<SearchInput bind:value={vulnSearch} placeholder="搜索 CVE、软件包、镜像、容器、堆栈..." class="h-8 w-80 text-sm" />
			<MultiSelectFilter
								...
								<div class="flex items-center gap-1.5">
									...
									{#if tagInfo.digestShort && tagInfo.pinnedRef}
										<button
											...
											title={copiedPinned === tagInfo.pinnedRef ? '已复制！' : '复制摘要锁定引用 (tag@sha256)'}
										>
