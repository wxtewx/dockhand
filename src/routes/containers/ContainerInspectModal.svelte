<script lang="ts">
	...
	async function connectToNetwork() {
		...
		try {
			...
			if (response.ok) {
				...
				toast.success(`已连接至 ${net?.name || '网络'}`);
				selectedNetwork = undefined;
				await fetchContainerInspect();
			} else {
				const data = await response.json();
				toast.error(data.details || '连接网络失败');
			}
		} catch (err) {
			toast.error('连接网络失败');
		} finally {
	...
	async function disconnectFromNetwork(networkId: string, networkName: string) {
		...
		try {
			...
			if (response.ok) {
				toast.success(`已断开与 ${networkName} 的连接`);
				await fetchContainerInspect();
			} else {
				const data = await response.json();
				toast.error(data.details || '断开网络连接失败');
			}
		} catch (err) {
			toast.error('断开网络连接失败');
		} finally {
			...
			<Dialog.Title class="flex items-center gap-2">
				...
				{#if composeStack && !loading}
					<Tooltip.Root>
						...
						<Tooltip.Content>
							<p class="text-xs whitespace-nowrap">打开堆栈 "{composeStack}"</p>
						</Tooltip.Content>
					</Tooltip.Root>
				{/if}
							...
							<div class="grid grid-cols-2 lg:grid-cols-5 gap-3">
								...
								<div class="p-3 border border-border rounded-lg">
									<div class="flex items-center gap-2 mb-2">
										...
										<span class="text-xs font-medium">进程</span>
										<button
											type="button"
						...
						<div class="space-y-2">
							...
							{#if isSharedNetworkMode}
								<p class="text-xs text-muted-foreground">
									网络命名空间通过 <code class="px-1 py-0.5 rounded bg-muted">{containerData.HostConfig?.NetworkMode}</code> 共享 — 无法附加额外网络。
								</p>
							...
							{:else}
								<p class="text-xs text-muted-foreground">未连接任何网络。</p>
							{/if}
										...
										<Select.Trigger class="flex-1 h-8 text-xs">
											...
											{:else}
												<span class="text-muted-foreground">
													{networksLoading ? '正在加载网络...' : unconnectedNetworks.length > 0 ? '加入网络...' : '暂无可用网络'}
												</span>
											{/if}
										</Select.Trigger>
										<Select.Content>
									...
									{#each Object.entries(containerData.NetworkSettings.Ports) as [containerPort, hostBindings]}
										...
										{:else}
											<div class="flex items-center gap-2 text-xs p-2 bg-amber-500/10 border border-amber-500/20 rounded">
												<code class="text-amber-600 dark:text-amber-400">已暴露</code>
								...
								{#each containerData.Mounts as mount}
									<div class="p-3 border border-border rounded-lg space-y-2">
										<div class="flex items-center justify-between">
											...
											{#if mount.Type === 'bind'}
												<span class="inline-flex items-center gap-1 rounded-full bg-amber-500/15 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-amber-600 dark:text-amber-400"><Folder class="h-3 w-3" />绑定挂载</span>
											{:else if mount.Type === 'volume'}
												<span class="inline-flex items-center gap-1 rounded-full bg-sky-500/15 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-sky-600 dark:text-sky-400"><HardDrive class="h-3 w-3" />数据卷</span>
											{:else}
					...
					<Tabs.Content value="env" class="space-y-4 overflow-auto">
						{#if containerData.divergence?.env?.length > 0}
							<div class="flex items-start gap-2 text-xs p-2.5 rounded border border-amber-500/30 bg-amber-500/5 text-amber-700 dark:text-amber-300">
								...
								<div class="min-w-0">
									共有 {containerData.divergence.env.length} 项环境变量与镜像不一致：
									<span class="font-mono">{containerData.divergence.env.join(', ')}</span>.
									下次更新时，未手动覆盖的镜像内置变量将同步更新为镜像内最新值；您自行设置的值会保留。使用移除并部署可将所有配置重置为镜像默认配置。
								</div>
							</div>
						{/if}
					...
					<Tabs.Content value="labels" class="space-y-3 overflow-auto">
						{#if containerData.divergence?.labels?.length > 0}
							<div class="flex items-start gap-2 text-xs p-2.5 rounded border border-amber-500/30 bg-amber-500/5 text-amber-700 dark:text-amber-300">
								<Info class="w-3.5 h-3.5 shrink-0 mt-0.5" />
								<div class="min-w-0">
									共有 {containerData.divergence.labels.length} 项标签与镜像不一致：
									<span class="font-mono">{containerData.divergence.labels.join(', ')}</span>.
									下次更新时，未手动覆盖的镜像内置标签将同步更新为镜像内最新值；您自行设置的标签会保留。使用 Remove & Deploy 可将所有配置重置为镜像默认配置。
								</div>
							</div>
						{/if}
						{#if containerData.Config?.Labels && Object.keys(containerData.Config.Labels).length > 0}
							...
							<div class="flex items-center gap-2">
								...
								<span class="text-xs text-muted-foreground shrink-0">
									{visibleLabels.length === allLabels.length
										? `共 ${allLabels.length} 项标签`
										: `已显示 ${visibleLabels.length} / ${allLabels.length}`}
								</span>
								<Button
									variant="outline"
									size="sm"
									...
									title={copiedAllLabels ? '已复制！' : '以 键=值 格式复制可见标签'}
								>
									{#if copiedAllLabels}
									...
									{#each visibleLabels as [key, value]}
										...
										<div class="text-xs p-2 rounded flex items-start gap-2 group {diverges ? 'bg-amber-500/10 border border-amber-500/30' : 'bg-muted'}">
											...
											<button
												...
												onclick={() => copyLabel(key, value)}
												class="shrink-0 p-1 rounded hover:bg-background/50 transition-colors opacity-0 group-hover:opacity-100 {copiedLabel === key ? '!opacity-100' : ''}"
												title={copiedLabel === key ? '已复制！' : '复制该标签'}
											>
												{#if copiedLabel === key}
													<Check class="w-3 h-3 text-green-500" />
												{:else}
							...
							{:else}
								<p class="text-sm text-muted-foreground">没有匹配 "{labelFilter}" 的标签</p>
							{/if}
						{:else}
