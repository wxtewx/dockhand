<script lang="ts">
	...
	function applyMemoryLimit() {
		...
		if (memoryLimit && bytes === undefined) {
			toast.error('内存数值格式错误 (例如：512m, 1g)');
			return;
		}
	...
	function applyNanoCpus() {
		...
		if (nanoCpus && n === undefined) {
			toast.error('CPU 限制数值格式错误 (例如：0.5, 1.5, 2)');
			return;
		}
...
<div class="space-y-5">
	...
	<div class="space-y-3">
		...
		<div class="space-y-1.5">
			<Label for="entrypoint" class="text-xs font-medium">入口程序 (选填)</Label>
		...
		{#if showResources}
			<div class="px-3 pb-3 space-y-3 border-t">
				<p class="text-xs text-muted-foreground pt-2">
					配置此容器的内存与 CPU 限制。
					{#if canApplyInPlace}
						The <CornerDownLeft class="w-3 h-3 inline-block -mt-0.5" /> 按钮可直接生效修改，无需重启容器。
					{/if}
				</p>
				...
				<div class="grid grid-cols-2 gap-3">
					...
					<div class="space-y-1.5">
						<Label for="cpuShares" class="text-xs font-medium">CPU 共享权重</Label>
						<div class="flex items-center gap-1.5">
							<Input id="cpuShares" bind:value={cpuShares} type="number" placeholder="例如：1024" class="h-9" />
				...
				<div class="grid grid-cols-2 gap-3">
					<div class="space-y-1.5">
						...
						<p class="text-xs text-muted-foreground">单位：微秒/每个调度周期</p>
					</div>
					<div class="space-y-1.5">
						<Label for="cpuPeriod" class="text-xs font-medium">CPU 调度周期</Label>
						<div class="flex items-center gap-1.5">
							<Input id="cpuPeriod" bind:value={cpuPeriod} type="number" placeholder="默认值: 100000" class="h-9" />
							{@render inlineApplyBtn('cpuPeriod', applyCpuPeriod)}
						</div>
						<p class="text-xs text-muted-foreground">调度周期时长，单位微秒</p>
					</div>
				</div>
			</div>
		{/if}
