<script lang="ts">
	...
	async function handleChown() {
		...
		try {
			...
			if (!res.ok) {
				throw new Error(data.error || '更改所有者失败');
			}

			toast.success('所有者已更改');
			showChownModal = false;
			chownEntry = null;
			loadDirectory(currentPath);
		} catch (err: any) {
			toast.error(err.message || '更改所有者失败');
		} finally {
			changingOwner = false;
		}
	}
...
<div class="flex flex-col h-full relative">
	...
	<Dialog.Root open={showCloseConfirm} onOpenChange={(o) => { if (!o) showCloseConfirm = false; }}>
		<Dialog.Content class="sm:max-w-md">
			<Dialog.Header>
				...
				<Dialog.Description>
					{editingFile?.name ?? '此文件'} 存在未保存的修改。是否在关闭前保存？
				</Dialog.Description>
			</Dialog.Header>
			<Dialog.Footer class="gap-2 sm:justify-between">
				<Button variant="outline" onclick={() => showCloseConfirm = false}>取消</Button>
				<div class="flex gap-2">
					<Button variant="destructive" onclick={forceCloseEditor}>放弃修改</Button>
	...
	{#if loadingPreview && !viewingFile}
		<LoadingState class="absolute inset-0 z-10 bg-background" label={`正在加载预览${previewingName ? ` - ${previewingName}` : ''}...`} />
	{:else if viewingFile}
		<div class="absolute inset-0 bg-background flex flex-col z-10">
			<div class="flex items-center justify-between p-2 border-b bg-muted/30">
				<div class="flex items-center gap-2 text-xs">
...
<Dialog.Root bind:open={showCreateModal}>
	<Dialog.Content class="max-w-sm">
		...
		<div class="space-y-4 py-4">
			...
			<div class="space-y-2">
				<Label for="create-owner">所有者 (选填)</Label>
				<Input
					id="create-owner"
					bind:value={createOwner}
					placeholder="例如：1000:1000 或 www-data"
					onkeydown={(e: KeyboardEvent) => { if (e.key === 'Enter') handleCreate(); }}
				/>
				<p class="text-xs text-muted-foreground">
					留空将使用容器的默认用户 (通常为 root).
				</p>
			</div>
			<p class="text-xs text-muted-foreground">
				将创建于：{currentPath}
...
<Dialog.Root bind:open={showChownModal}>
	<Dialog.Content class="max-w-md">
		<Dialog.Header>
			<Dialog.Title>更改所有者</Dialog.Title>
		</Dialog.Header>
		<div class="space-y-4 py-4">
			{#if chownEntry}
				<p class="text-sm text-muted-foreground">{chownEntry.name}</p>
				{#if chownEntry.owner}
					<p class="text-xs text-muted-foreground">当前: {chownEntry.owner}</p>
				{/if}
			{/if}
			...
			<div class="space-y-2">
				...
				<Input
					...
					placeholder="例如 1000:1000 or www-data"
					onkeydown={(e: KeyboardEvent) => { if (e.key === 'Enter') handleChown(); }}
				/>
				<p class="text-xs text-muted-foreground">
					用户或者 用户:用户组。即使容器内不存在该名称，数字 ID 依然可以生效。
				</p>
			</div>
