...
<Dialog.Root bind:open>
	<Dialog.Content class="max-w-6xl h-[90vh] sm:h-[85vh] flex flex-col overflow-hidden">
		<Dialog.Header>
			...
			<Dialog.Description class="sr-only">浏览快照 {snapshotId ? String(snapshotId).slice(0, 8) : ''} 内的文件。</Dialog.Description>
		</Dialog.Header>
				...
				{:else if metadata}
					<div class="max-w-2xl space-y-5 px-1">
						...
						<section class="space-y-2">
							...
							<dl class="grid grid-cols-[130px_1fr] gap-y-1.5 text-sm">
								...
								{#if metadata.type === 'stack'}
									...
									<dd>{metadata.hasStackFiles ? '已采集' : '未采集'}</dd>
								{/if}
							</dl>
						</section>
						...
						{#if metadata.type === 'stack' && metadata.stack?.fileList?.length}
							<section class="space-y-2 border-t pt-4">
								<h4 class="text-xs font-semibold text-muted-foreground uppercase tracking-wide flex items-center gap-1.5"><FileText class="w-3.5 h-3.5" />堆栈文件 ({metadata.stack.fileList.length})</h4>
								<div class="space-y-1">
						...
						{#if metadata.type === 'stack' && metadata.stack?.secretKeys?.length}
							<section class="space-y-2 border-t pt-4">
								<h4 class="text-xs font-semibold text-muted-foreground uppercase tracking-wide flex items-center gap-1.5"><KeyRound class="w-3.5 h-3.5" />密钥 ({metadata.stack.secretKeys.length})</h4>
