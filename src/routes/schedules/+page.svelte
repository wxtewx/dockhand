			...
			<Dialog.Title class="flex items-center gap-2">
				...
				{#if selectedExecution}
					<span class="text-muted-foreground font-normal">
						({#if selectedExecution.scheduleType === 'container_update'}容器更新{:else if selectedExecution.scheduleType === 'env_update_check'}环境更新检查{:else if selectedExecution.scheduleType === 'git_stack_sync'}Git 堆栈同步{:else if selectedExecution.scheduleType === 'backup'}备份{:else if selectedExecution.scheduleType === 'repo_prune'}仓库清理{:else if selectedExecution.scheduleType === 'repo_check'}仓库检查{:else if selectedExecution.scheduleType === 'repo_verify'}数据校验{:else if selectedExecution.scheduleType === 'stack_deploy'}堆栈部署{:else if selectedExecution.scheduleType === 'deploy_log_reconcile'}部署日志同步{:else}系统任务{/if})
					</span>
				{/if}
			</Dialog.Title>
			{#if selectedExecution}
