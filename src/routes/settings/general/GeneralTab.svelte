...
`;
	...
	function handleDeployLogReconcileCronChange(cron: string) {
		...
		toast.success('部署日志同步 cron 表达式已更新');
	}
	...
	function handleDeployLogReconcileEnabledChange() {
		...
		toast.success(newState ? '部署日志同步已启用' : '部署日志同步已停用');
	}
		...
		<div class="space-y-4">
							...
							<div class="space-y-1">
								<div class="flex items-center gap-3">
									...
									<Tooltip.Root>
										...
										<Tooltip.Content side="top" class="w-96 max-w-[90vw]">
											<p>从<a href="https://selfh.st" target="_blank" rel="noopener" class="underline"> selfh.st </a>获取应用 Logo 作为容器图标，根据镜像名称自动匹配。Logo 仅拉取一次并在本地缓存，浏览器不会访问外部 CDN。默认关闭。Logo 基于 CC BY 4.0 协议；产品名称与商标归各自所有者所有，仅用于标识用途，不代表相关方认可本项目。</p>
										</Tooltip.Content>
									</Tooltip.Root>
									<TogglePill
										...
										onchange={(checked) => {
											...
											toast.success(checked ? '已启用 selfh.st 图标' : '已禁用 selfh.st 图标');
										}}
										disabled={!$canAccess('settings', 'edit')}
									/>
								</div>
								<p class="text-xs text-muted-foreground">容器自动应用 Logo，从 selfh.st 获取并本地缓存</p>
							</div>
							<div class="space-y-1">
								<div class="flex items-center gap-3">
									<Label>显示 "更新公告"</Label>
						...
						<div class="space-y-4">
							...
							{#if $authStore.authEnabled}
								<div class="text-xs text-muted-foreground flex items-start gap-1.5 mt-2 p-2 bg-muted/50 rounded-md">
									...
									<div>
										<p>这些是<strong>新用户的默认设置</strong> — 不会更改您当前的界面显示。如需自定义<em>您本人</em>的应用外观，请前往您的<a href="/profile" class="text-primary hover:underline">个人资料</a>中的主题设置。</p>
									</div>
								</div>
							{/if}
						</div>
							...
							<div class="flex items-center gap-3">
								...
								<ToggleSwitch
									...
									onchange={(newFormat) => {
										...
										toast.success(`已将时间格式设置为 ${newFormat === '12h' ? '12-小时制(上午/下午)' : '24-小时制'}`);
									}}
									disabled={!$canAccess('settings', 'edit')}
								/>
							</div>
							<p class="text-xs text-muted-foreground">应用内各处所使用的时钟显示</p>
						</div>
						<div class="space-y-1">
							<div class="flex items-center gap-3">
								<Label>日期格式</Label>
							...
							<p class="text-xs text-muted-foreground">应用内各处所使用的日期显示</p>
						</div>
					</div>
				</div>
			...
			<Card.Root>
				<Card.Header>
					...
					<p class="text-xs text-muted-foreground">选择哪些堆栈操作会打开完整的 compose-log 弹窗。未勾选的操作将静默执行，仅弹出提示；如果操作失败，其日志仍会自动打开。</p>
				</Card.Header>
				<Card.Content>
			...
			<Card.Root>
				...
				<Card.Content class="space-y-4">
					...
					<div class="space-y-1 pt-2 border-t">
						<div class="flex items-center gap-3">
							...
							<Tooltip.Provider delayDuration={100}>
								<Tooltip.Root>
									...
									<Tooltip.Portal>
										<Tooltip.Content side="right" sideOffset={8} class="!w-80">
											Dockhand 的每一次部署都会在磁盘保留日志文件，并关联到【部署】标签页中的运行记录。该任务按计划执行以保持两者同步：删除已移除部署记录对应的孤立日志文件，并标记日志文件丢失的运行记录 (让【部署】标签页显示“日志不可用”而非空白)。此任务不会删除部署运行记录本身。
										</Tooltip.Content>
									</Tooltip.Portal>
								</Tooltip.Root>
							</Tooltip.Provider>
						...
						<p class="text-xs text-muted-foreground">保持部署日志文件与部署记录同步：删除已不存在运行记录对应的日志，并标记日志丢失的运行记录 (不会删除运行记录)。</p>
							...
							<div class="flex items-center gap-3">
								...
								<Tooltip.Root>
									...
									<Tooltip.Content class="w-80">
										<p class="text-xs">
											控制仪表盘上多选环境标签时如何筛选环境。
											<strong>"任意"</strong>: 显示至少含有一个已选标签的环境。
											<strong>"全部"</strong>: 仅显示同时拥有所有已选标签的环境。
										</p>
									</Tooltip.Content>
								</Tooltip.Root>
								<ToggleSwitch
