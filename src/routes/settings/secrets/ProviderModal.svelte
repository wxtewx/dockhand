<script lang='ts' module>
	...
	export const PROVIDER_FIELDS: Record<string, ProviderField[]> = {
		...
		infisical: [
			...
			{ key: 'token', label: '访问令牌', type: 'password', required: false, placeholder: 'st...', hint: '静态服务/访问令牌。留空则使用下方的 Universal Auth (客户端 ID + 密钥)。' },
			{ key: 'clientId', label: 'Universal Auth 客户端ID', type: 'text', required: false, placeholder: '机器身份客户端 ID', hint: '机器身份客户端 ID。与客户端密钥配对使用；使用静态令牌时请留空。' },
			{ key: 'clientSecret', label: 'Universal Auth 客户端密钥', type: 'password', required: false, placeholder: '机器身份客户端密钥', hint: '机器身份客户端密钥。通过 Universal Auth 换取短期令牌。' },
		...
		keepass: [
			{ key: 'databasePath', label: '数据库路径', type: 'text', required: true, placeholder: '/secrets/passwords.kdbx', hint: 'Dockhand 容器内识别到的 .kdbx 文件绝对路径 (以只读方式挂载)。容器中必须安装 keepassxc-cli。' },
			{ key: 'password', label: '主密码', type: 'password', required: false, requiredWhen: (c) => !(c.keyFilePath ?? '').trim(), placeholder: '数据库主密码', hint: '数据库主密码。如果提供密钥文件 (单独或搭配使用)，该项可不填。' },
			{ key: 'keyFilePath', label: '密钥文件路径', type: 'text', required: false, placeholder: '/secrets/db.keyx', hint: '可选，容器内识别到的数据库密钥文件绝对路径。' },
		],
	};
	...
	export const BULK_SELECTOR_FIELDS: Record<string, BulkSelectorField> = {
		...
		'azure-kv': {
			label: 'Key Vault',
			placeholder: '填入任意值即可启用批量拉取',
			hint: '批量加载保管库内所有密钥。填入任意值启用该功能；留空则仅解析内联 azurekv:// 引用。'
		},
		'keepass': {
			label: '分组',
			placeholder: '例如：dockhand (留空则仅使用内联引用)',
			hint: '批量加载该分组下所有条目，格式为 ENV=<条目密码>。留空则仅解析内联 keepass:// 引用。'
		}
	};
</script>
				...
				<div class="min-h-16">
					...
					{:else if formType === 'keepass'}
						<p class="flex items-start gap-2 text-xs text-muted-foreground">
							<Info class="h-3.5 w-3.5 shrink-0 mt-0.5 text-amber-500" />
							<span>
								KeePassXC 需要在容器内安装官方 <code>keepassxc-cli</code> 客户端 (或通过 <code>DOCKHAND_KEEPASSXC_CLI_PATH</code> 绝对路径覆盖进程位置)，
								并且需要将 <code>.kdbx</code> 数据库以只读方式挂载到容器内的上述路径。
								同时支持分组批量拉取以及内联 <code>keepass://</code> 引用。
							</span>
						</p>
					{:else}
						<p class="flex items-start gap-2 text-xs text-muted-foreground">
