...
jsons://hostname/webhook/path"
					...
					<p class="text-xs text-muted-foreground">
						支持 Discord、Slack、Mattermost、Telegram、ntfy、Gotify、Pushover、Bark、Signal (通过 signal-cli-rest-api), Microsoft Teams (通过工作流) 以及通用 JSON 格式。也可使用 <code>apprise://</code> 转发至自建的 <a href="https://github.com/caronc/apprise-api" target="_blank" rel="noopener">caronc/apprise-api</a> 服务，以此兼容 Apprise 上游支持的全部通知渠道。
						</p>
					</div>
				</div>
			{/if}
