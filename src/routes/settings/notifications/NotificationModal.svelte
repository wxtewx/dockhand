...
zabbixs://hostname/api_jsonrpc.php?token=TOKEN&amp;host=HOST&amp;key=ITEM_KEY"
					...
					<p class="text-xs text-muted-foreground">
						支持 Discord、Slack、Mattermost、Telegram、ntfy、Gotify、Pushover、Bark、Signal (通过 signal-cli-rest-api)、Microsoft Teams (通过 Workflows)、Zabbix (通过 history.push) 以及通用 JSON。也可使用 <code>apprise://</code> 转发至自建的 <a href="https://github.com/caronc/apprise-api" target="_blank" rel="noopener">caronc/apprise-api</a> 服务端，以支持 Apprise 上游兼容的全部通知提供商。
					</p>
					<p class="text-xs text-muted-foreground">
						Zabbix 7.x:
