						...
						{#if usesStackPath(formConnectionType)}
							<div class="space-y-2">
											...
											{:else}
												<div class="space-y-2">
													...
													<p class="text-muted-foreground">
														...
														<span class="font-medium text-foreground">远程主机</span>
														<span class="font-medium text-foreground">每次部署时</span>，以便备份
														助手能够读取 compose 和配置文件，并将相对绑定挂载
														(<code class="bg-muted px-1 rounded">./data</code>) 重写为该主机路径，使其在远程守护进程上正常解析。设置完成后，请重新部署一次堆栈，将文件暂存至该位置。
													</p>
													<p class="text-muted-foreground">
								...
								<p class="text-xs text-muted-foreground">
									...
									{:else}
										Dockhand 在远程主机上存放此堆栈文件的绝对路径，以便对编排文件与配置进行备份，并且相对绑定挂载可在远程守护进程上正常解析。留空则仅使用绝对路径或命名数据卷。
										<span class="text-foreground font-medium">将于下次部署生效</span> - 设置完成后，请重新部署每个堆栈，让 Dockhand 将文件暂存到此位置。
									{/if}
								</p>
							</div>
						{/if}
...
<Dialog.Root bind:open={memWarnOpen}>
	<Dialog.Content class="max-w-md">
		...
		<div class="text-sm text-muted-foreground space-y-3">
			<p>
				此主机的内核已禁用 cgroup 内存计量，因此 Docker 无法上报每个容器的内存信息 — 所有容器都将显示为
				<span class="font-medium text-foreground whitespace-nowrap">0&nbsp;B</span>。树莓派以及部分 ARM 开发板常会出现该情况。
			</p>
			<p>
				该问题需要在内核启动配置中修复，无法通过 Dockhand 解决。查看文档：
		...
		<Dialog.Footer>
			<Button onclick={() => (memWarnOpen = false)}>确定</Button>
		</Dialog.Footer>
	</Dialog.Content>
</Dialog.Root>
