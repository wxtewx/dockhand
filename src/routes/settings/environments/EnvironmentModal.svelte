					...
					<Tabs.Content value="general" class="space-y-4 mt-0 h-full">
							...
							<div class="space-y-2">
								<div class="flex items-center gap-1.5">
									<Label for="edit-env-remote-stacks-dir">
										{isHawserConn(formConnectionType) ? '远程堆栈路径 (用于备份)' : '远程堆栈目录'}
										<span class="text-muted-foreground font-normal">(选填)</span>
									</Label>
									<Tooltip.Root>
										...
										<Tooltip.Content class="w-80 z-[200]" side="right">
											...
											{:else}
												<div class="space-y-2">
													<p class="font-medium">该路径的作用说明</p>
													<p class="text-muted-foreground">
														直连模式守护进程与 Dockhand 无共享文件系统。填写该路径后，Dockhand 会将完整堆栈目录 (编排文件、<code class="bg-muted px-1 rounded">.env</code>、
														绑定挂载文件 (<code class="bg-muted px-1 rounded">./config</code>、
														<code class="bg-muted px-1 rounded">./data</code>)) 全部同步至远程主机的
														<code class="bg-muted px-1 rounded">&lt;该路径&gt;/&lt;堆栈名称&gt;</code>
														目录，并通过 <code class="bg-muted px-1 rounded">--project-directory</code> 参数在此执行编排部署。
													</p>
													<p class="text-muted-foreground">
														该配置对普通堆栈与Git同步堆栈均生效，可保证相对路径绑定正常解析，让远程直连守护进程上的所有堆栈均可被完整备份。留空则仅支持命名数据卷或绝对路径挂载。
													</p>
												</div>
											{/if}
										</Tooltip.Content>
								...
								<p class="text-xs text-muted-foreground">
									...
									{:else}
										远程主机上 Dockhand 部署前暂存堆栈文件的绝对路径。存在相对路径绑定的堆栈需要填写此参数，确保远程直连守护进程下的堆栈能够正常备份。
									{/if}
								</p>
							</div>
						{/if}
						...
						{#if formConnectionType === 'hawser-standard'}
							...
							<div class="space-y-2">
								...
								<Input id="edit-env-hawser-token" type="password" bind:value={formHawserToken} placeholder="用于代理身份验证的令牌" oninput={() => generatedStandardToken = null} />
