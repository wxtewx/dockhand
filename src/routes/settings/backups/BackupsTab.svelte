	...
	async function runRepoTask(destId: number, task: string) {
		...
		try {
			...
			if (data.success) {
				...
				toast.success(data.output ? getResticText(data.output) : `${task} 执行完成`);
			} else {
				toast.error(data.error ? getResticText(data.error) : `${task} 执行失败`);
			}
		} catch (err: any) {
			toast.error(err.message ? getResticText(err.message) : `${task} 执行失败`);
		} finally {
			runningTask = null;
		}
	}
