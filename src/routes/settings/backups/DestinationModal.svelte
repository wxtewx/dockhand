	...
	const backendTypes: BackendType[] = [
		{
			...
			fields: [
				{ key: 'path', label: '路径', placeholder: '/app/local-backups/myrepo', optional: true }
			],
			buildRepo: (f) => f.path || '',
	...
	<Dialog.Content class="max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
		...
		{#if selectedBackend.value === 'local'}
			<div class="flex items-start gap-2 p-2.5 mt-4 rounded-md bg-amber-500/10 border border-amber-500/20 text-xs text-amber-600 dark:text-amber-400">
				...
				<span>请填写 Dockhand 在容器内部识别的路径 (即备份卷挂载的容器侧路径，例如 <code>/app/local-backups/...</code>)，不要填写主机路径。本地路径仓库仅适用于本地 Docker 主机，或同一主机上同部署的 socket-proxy。对于远程主机，请使用 S3、REST 或其他远程后端。</span>
			</div>
		{/if}
		...
		<div class="border-t pt-3 mt-2 space-y-3">
			<span class="text-xs font-medium text-muted-foreground uppercase tracking-wide">TLS 证书 (可选)</span>
			<p class="text-xs text-muted-foreground -mt-1">
				适用于使用私有/自签名 CA 的 HTTPS 后端服务。留空则使用系统信任存储。
			</p>
			<div class="grid grid-cols-2 gap-6">
				<div class="space-y-1">
					<div class="flex items-center justify-between">
						<Label for="dest-cacert">CA 证书</Label>
						<div class="flex gap-1">
							{#if isEditing && hadCacert && !formCacert}
								<Button variant="outline" size="sm" class="h-7 px-2 text-xs text-destructive" onclick={() => { hadCacert = false; }}>清除</Button>
							{/if}
							<Button variant="outline" size="sm" class="h-7 px-2 text-xs" onclick={() => caFileInput?.click()}>
								<Upload class="mr-1 h-3 w-3" />上传
							</Button>
						</div>
					</div>
					...
					<Textarea
						...
						placeholder={isEditing && hadCacert ? '(已存储 CA 证书 - 留空可保留原有内容)' : '-----BEGIN CERTIFICATE-----'}
						class="field-sizing-fixed max-h-40 resize-y overflow-auto font-mono text-xs"
					/>
					<p class="text-xs text-muted-foreground">用于校验后端 TLS 证书 (restic <code class="font-mono">RESTIC_CACERT</code>).</p>
				</div>
				<div class="space-y-1">
					<div class="flex items-center justify-between">
						<Label for="dest-client-cert">客户端证书 (mTLS)</Label>
						<div class="flex gap-1">
							{#if isEditing && hadTlsClientCert && !formTlsClientCert}
								<Button variant="outline" size="sm" class="h-7 px-2 text-xs text-destructive" onclick={() => { hadTlsClientCert = false; }}>清除</Button>
							{/if}
					...
					<Textarea
						...
						placeholder={isEditing && hadTlsClientCert ? '(已存储客户端证书 - 留空可保留原有内容)' : '证书+私钥合并至单个 PEM'}
						class="field-sizing-fixed max-h-40 resize-y overflow-auto font-mono text-xs"
					/>
					<p class="text-xs text-muted-foreground">双向 TLS 所需的证书+私钥合并 PEM (restic <code class="font-mono">RESTIC_TLS_CLIENT_CERT</code>).</p>
				</div>
			</div>
		</div>
		{/if}
