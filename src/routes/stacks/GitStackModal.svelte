<script lang="ts">
	...
	async function loadSecretProviderBindingForStack(stackName: string) {
		try {
			...
			console.warn('加载 git 堆栈的密钥提供程序绑定失败:', e);
		}
	}
	...
	async function saveGitStack(deployAfterSave: boolean = false) {
		...
		if (!trimmedStackName) {
			...
			errors.stackName = '堆栈名称必须为小写字符，首字符为字母或数字，仅允许使用字母、数字、连字符与下划线';
			hasErrors = true;
		}
		...
		if (formWebhookEnabled && !formWebhookSecret.trim() && !$page.data.allowSecretlessWebhook) {
			errors.webhookSecret = '启用 webhook 时必须配置 webhook 密钥';
			hasErrors = true;
		}
		...
		try {
			...
			if (deployAfterSave) {
				outputTitle = `正在部署 ${formStackName.trim()}`;
				outputLines = [];
				outputRunning = true;
				outputOk = undefined;
				outputMs = undefined;
	...
	>
		...
		{#if gitStack}
			<div class="flex items-center gap-1 border-b border-zinc-200 px-5 dark:border-zinc-700 flex-shrink-0">
				...
				>
					<Settings2 class="h-3.5 w-3.5" /> 设置
				</button>
				<button
					type="button"
				...
				>
					<History class="h-3.5 w-3.5" /> 部署记录
		...
		<div bind:this={containerRef} class="flex-1 min-h-0 flex {isDraggingSplit ? 'select-none' : ''}">
						...
						{#if formRepoMode === 'existing' && selectedRepo}
							<div class="space-y-2">
								...
								<BranchCombobox
									...
									placeholder="仓库默认 ({selectedRepo.branch})"
									clearLabel="仓库默认 ({selectedRepo.branch})"
									onchange={(v) => { formBranch = v; }}
									onclear={() => { formBranch = null; }}
								/>
								<p class="text-xs text-muted-foreground">此堆栈部署所使用的分支。留空则跟随仓库配置的分支 ({selectedRepo.branch})。</p>
							</div>
						{/if}
					{:else}
						<div class="space-y-3 p-3 border rounded-md bg-muted/30">
							...
							<div class="grid grid-cols-2 items-start gap-3">
								...
								<div class="space-y-2">
									...
									<p class="text-xs text-muted-foreground">私有仓库使用的 SSH 密钥或访问令牌。</p>
								</div>
							</div>
						</div>
					{/if}
			...
			<div class="space-y-2">
				...
				{:else}
					<p class="text-xs text-muted-foreground">这将是已部署堆栈的名称</p>
				{/if}
			</div>
			...
			{#if gitStack && selectedRepo}
				<div class="space-y-2">
					...
					<BranchCombobox
						...
						onclear={() => { formBranch = null; }}
					/>
					<p class="text-xs text-muted-foreground">该堆栈用于部署的分支。留空将跟随仓库上配置的分支 ({selectedRepo.branch})。</p>
				</div>
			{/if}
					...
					{#snippet headerActions()}
						<div class="flex items-center gap-0.5">
							...
							<Tooltip.Root>
								...
								<Tooltip.Content>
									<div class="w-64">
										<p class="text-xs">克隆仓库，并从 (Compose 目录下的)<code class="bg-muted px-1 rounded">.env</code> 文件以及额外环境文件 (如有指定) 载入环境变量，便于查看可覆盖的变量。</p>
									</div>
								</Tooltip.Content>
							</Tooltip.Root>
						</div>
...
<IconPickerModal bind:open={showIconPicker} value={formIcon} onselect={onIconSelect} title="选择堆栈图标" />
