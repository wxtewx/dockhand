<script lang="ts">
	...
	async function confirmChangeLocation() {
		...
		try {
			...
			if (!response.ok) {
				const data = await response.json();
				throw new Error((typeof data.error === 'string' ? data.error : data.message) || '移动文件失败');
			}
	...
	async function handleCreate(start: boolean = false, persistenceAcked = false, deployOptions?: DeployOptions) {
		...
		try {
			...
			if (start) startOutput(`正在启动 ${newStackName.trim()}`);
	...
	async function handleSave(restart = false, moveFromDir: string | null | undefined = undefined, persistenceAcked = false, deployOptions?: DeployOptions) {
		...
		try {
			...
			if (restart) startOutput(`重新部署 ${stackName}`);
				...
				<div bind:this={containerRef} class="flex-1 min-h-0 flex flex-col {isDraggingSplit ? 'select-none' : ''}">
					{#if activeTab === 'editor'}
						...
						<div class="flex items-center border-b border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/30">
							{#if readonly}
								<span class="ml-4 flex shrink-0 items-center gap-1 rounded-full border border-purple-500/30 bg-purple-500/10 px-2 py-0.5 text-2xs font-medium text-purple-600 dark:text-purple-400" title="这是由 Git 管理的堆栈 — Dockhand 以只读模式展示其 Compose 配置；请在代码仓库中编辑。">
				...
				{#if !readonly && mode === 'create'}
					...
					<div class="inline-flex">
						...
						<RedeployPopover
							...
							reason={hasBuildSection ? '自动勾选：此编排文件包含 build: 配置段' : undefined}
