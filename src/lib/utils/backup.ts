...
export function bulkDeleteToast(r: BulkDeleteResult): { type: 'success' | 'error'; message: string } {
	if (r.failed) return { type: 'error', message: r.deleted > 0 ? `已删除 ${r.deleted} 个，但部分快照无法删除` : '无法删除快照' };
	if (r.deleted === 0 && r.skipped > 0) return { type: 'error', message: `${r.skipped} 个快照无法删除 (无所有权或无访问权限)` };
	return { type: 'success', message: `已删除 ${r.deleted} 个快照${r.skipped > 0 ? ` (跳过 ${r.skipped} 个)` : ''}` };
}
