...
const LabelMaps = {
  ...
  restic: {
    ...
    "repository opened successfully, password is correct": "成功打开备份仓库，密码校验通过",
    "using temporary cache in": "使用临时缓存：",
    "loading indexes...": "正在加载索引",
    "loading all snapshots...": "正在加载全部快照",
    ...
    "the unlock command can be used to remove stale locks": "可使用 unlock 命令清理过期仓库锁",
    ...
    "Important: Please keep this repository password safe": "重要提示：请妥善保管仓库密码",
    "load indexes": "加载索引",
    "check all packs": "校验所有数据包",
    "check snapshots, trees and blobs": "校验快照、目录树与数据块",
    ...
    "finding data that is still in use for": "检索正在被快照引用的数据，快照数量：",
    "searching used packs...": "检索正在使用的数据包",
    "collecting packs for deletion and repacking": "收集待删除/重新打包的数据包",
    "to repack": "需要重新打包",
    "this removes": "将移除",
    "to delete": "待删除数据",
    ...
    "Fatal: unable to create lock in backend: repository is already locked": "严重错误：无法创建锁，备份仓库已被占用",
...
export function getLabelText(
  ...
  if (type === 'health') {
    ...
    return FlatLabelMap[lowerVal] || '启动中';
  }

  if (type === 'restartPolicy') {
    const lowerVal = text.toLowerCase();
    return FlatLabelMap[lowerVal] || '无';
  }

  const resticEntries = Object.entries(LabelMaps.restic).sort((a,b) => b[0].length - a[0].length);
  for (const [en, cn] of resticEntries) {
  ...
  if (FlatLabelMap[text.trim()]) {
    return FlatLabelMap[text.trim()];
  }
