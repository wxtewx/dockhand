...
export const RULES: RuleDefinition[] = [
	...
	{
		id: 'UNUSED_VOLUME',
		description: '顶层命名数据卷已定义，但没有任何服务挂载该数据卷',
		group: 'correctness',
		...
		check(p) {
			...
			for (const vol of definedNames) {
				...
				out.push({
					...
					message: `命名数据卷 "${vol}" 已定义，但没有服务挂载它`,
					hint: `在服务的 volumes: 配置中挂载该数据卷，若不再需要则从顶层 volumes: 中移除。`,
	...
	},
	{
		...
		description: '服务未配置健康检查，Docker 无法判断该服务是否正常工作',
		group: 'reliability',
		defaultSeverity: 'info',
		check(p) {
			const out: RuleFinding[] = [];
			for (const [name, svc] of Object.entries(services(p))) {
				...
				out.push({
					...
					service: name,
					message: `"${name}" 未配置健康检查 - 即使容器内应用已停止响应，Docker 仍会标记容器为运行中`,
					hint: '添加健康检查，使得重启策略和 depends_on: condition: service_healthy 可以响应卡死的容器。镜像可能已内置健康检查。',
					line: p.lineOf(['services', name])
				});
			}
			return out;
