...
function getOrCreateKey(): Buffer {
	...
	try {
		...
		const msg = error instanceof Error ? error.message : String(error);
		cachedKey = null;
		console.error(`[加密] 无法将新生成的加密密钥保存至 ${keyPath}: ${msg}`);
		throw new Error(
			`无法将加密密钥保存至 ${keyPath}; 拒绝使用临时密钥运行，重启后密钥将会丢失 (请检查数据目录权限/磁盘空间): ${msg}`
		);
	}
...
export function decryptStrict(value: string | null | undefined): string | null {
	...
	if (result === value) {
		throw new Error('解密存储凭据失败 (密钥错误或密钥已轮换)。拒绝直接使用密文作为运行密钥。');
	}
	return result;
}
...
export async function migrateCredentials(): Promise<void> {
	...
	if (pendingKeyRotation) {
		...
		try {
			...
			cachedKey = oldKey;
			migrated = 0;
			console.error('[加密] 密钥轮换失败，事务已回滚 — 凭据仍使用旧密钥加密:', err instanceof Error ? err.message : String(err));
			throw err;
		}
