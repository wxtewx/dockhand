...
async function computeSyncDeletionPlan(options: {
	...
	if (canSkipTreeRehash(changedInComposeDir, manifestSize, previousManifest.commit, diffBaselineCommit)) {
		console.log(`${logPrefix} 删除同步：compose 目录无变更，跳过对 ${manifestSize} 个文件的完整重新哈希`);
		return { plan: { toDelete: [], skipped: [] }, newFiles: { ...previousManifest.files }, previousManifest };
	}

	const newFiles = hashDirFiles(composeDir);

	console.log(`${logPrefix} 删除同步：清单包含 ${manifestSize} 个文件${manifestSize === 0 ? ' (f首次同步 — 不会删除任何内容)' : ''}`);
...
export async function deployGitStack(
	...
	if (!syncResult.success) {
		...
		if (!(syncResult.error ?? '').includes('Sync already in progress')) {
			try {
				...
				failRecorder.line(`fatal: ${syncResult.error ?? 'git 同步失败'}`);
				await failRecorder.end(false, undefined, syncResult.error ?? 'git 同步失败');
			} catch (e) {
				console.error(`${logPrefix} 记录 git 同步失败信息时出错:`, e);
			}
		}
	...
	console.log(`${logPrefix} 是否强制重建:`, forceRecreate, `(updated=${syncResult.updated}, forceRedeploy=${gitStack.forceRedeploy})`);
	console.log(`${logPrefix} 部署时是否构建:`, gitStack.buildOnDeploy);
	console.log(`${logPrefix} 是否重新拉取镜像:`, gitStack.repullImages);
	...
	try {
		...
		console.error(`${logPrefix} 读取运行记录脱敏所需环境变量失败 (部署继续执行):`, e);
	}
	...
	const recorder = await createRunRecorder({
		...
		console.error(`${logPrefix} 创建部署运行记录器失败 (继续执行，不生成记录):`, e);
		return null;
	});
	...
	try {
		...
		try {
			...
			console.error(`${logPrefix} 部署出错时关闭运行记录器失败 (原始错误保留):`, e);
		}
		throw error;
	}
	...
	try {
		...
		console.error(`${logPrefix} 关闭运行记录器失败 (部署结果不受影响):`, e);
	}
...
export async function deployGitStackWithProgress(
	...
	const recorder = await createRunRecorder({
		...
		console.error('创建 git 部署运行记录器失败 (继续执行，不生成记录):', e);
		return null;
	});
	...
	try {
		// Step 1: Connecting
		onProgressAndRecord({ status: 'connecting', message: '正在连接仓库...', step: 1, totalSteps });
		await updateGitStack(stackId, { syncStatus: 'syncing', syncError: null });
		...
		// Step 2: Cloning
		onProgressAndRecord({ status: 'cloning', message: '正在克隆仓库...', step: 2, totalSteps });
		...
		onProgressAndRecord({ status: 'fetching', message: `正在获取分支 ${effectiveBranch}...`, step: 3, totalSteps });
		...
		onProgressAndRecord({ status: 'reading', message: `正在读取 ${gitStack.composePath}...`, step: 4, totalSteps });
		const composePath = repoFilePath(repoPath, gitStack.composePath, "Compose path");
		if (!existsSync(composePath)) {
			throw new Error(`未找到 Compose 文件：${gitStack.composePath}`);
		}
		...
		onProgressAndRecord({ status: 'deploying', message: `文件变更: ${changeTable[0]}`, step: 5, totalSteps });
		...
		// Uses `docker compose up -d --remove-orphans` which only recreates changed services
		onProgressAndRecord({ status: 'deploying', message: `正在部署 ${gitStack.stackName}...`, step: 5, totalSteps });
		...
		try {
			...
			console.error('读取运行记录脱敏所需环境变量失败 (部署继续执行):', e);
		}
