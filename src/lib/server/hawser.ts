...
export async function handleEdgeMetrics(
	...
	try {
		...
		const errorMsg = error instanceof Error ? error.message : String(error);
		console.error('[Hawser] 保存监控指标出错:', errorMsg);
	}
}
...
export async function revokeHawserToken(tokenId: number): Promise<void> {
	...
	for (const connection of edgeConnections.values()) {
		if (connection.tokenId === tokenId) {
			console.log(`[Hawser] 关闭环境 ${connection.environmentId} 的边缘连接 — 该令牌 (${tokenId}) 已被撤销`);
	...
	return new Promise((resolve, reject) => {
		...
		if (globalThis.__hawserSendMessage) {
			...
			try {
				...
				console.error(`[Hawser Edge] 发送消息出错:`, errorMsg);
				connection.pendingRequests.delete(requestId);
				if (streaming) {
					connection.pendingStreamRequests.delete(requestId);
				}
...
export function sendEdgeStreamRequest(
	...
	if (globalThis.__hawserSendMessage) {
		...
		try {
			...
			const errorMsg = sendError instanceof Error ? sendError.message : String(sendError);
			console.error(`[Hawser Edge] 发送流式消息出错:`, errorMsg);
			connection.pendingStreamRequests.delete(requestId);
