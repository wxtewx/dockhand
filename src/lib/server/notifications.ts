...
async function sendGotify(appriseUrl: string, payload: NotificationPayload): Promise<NotificationResult> {
	...
	if (!parsed) {
		return { success: false, error: 'Gotify 链接格式无效，格式要求：gotify://hostname/token' };
	}
...
async function sendBark(appriseUrl: string, payload: NotificationPayload): Promise<NotificationResult> {
	...
	if (!path.includes('/')) {
		if (!path) {
			return { success: false, error: 'Bark 链接格式无效。合法格式: bark://device_key, bark://host/device_key 或 barks://host/device_key' };
		}
