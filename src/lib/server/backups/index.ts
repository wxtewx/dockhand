...
> {
	...
	if (resolution.kind === 'unknown') {
		...
		if (directRemote && !remoteStacksDir && dockhandStackDir) {
			console.log(`[备份] 堆栈目录执行方案 "${targetName}"：归档打包模式 (直连远程环境，未配置remote_stacks_dir)，将从Dockhand本地目录 ${dockhandStackDir} 读取编排与配置文件进行打包`);
			...
			const { STACKDIR_VOLUME_KEY } = await import('./stackdir-plan');
			{
				try {
					...
					if (stackDirTar) {
						...
						console.log(`[备份] 堆栈文件 "${targetName}": TAR模式 - 从目录 ${stackDir} 流式传输 ${stackDirStreamSources.length} 个文件至 /volumes/${STACKDIR_VOLUME_KEY}`);
					} else {
						console.log(`[备份] 堆栈文件 "${targetName}": 通过主机绑定挂载捕获至 /volumes/${STACKDIR_VOLUME_KEY} - 共列出 ${listed.length} 个文件，已排除 ${excludeRelDirs.length} 个绑定目录`);
					}
				} catch (e) {
...
async function materialiseStackFiles(destination: any, snapId: string, stackName: string, targetPath: string, overwrite: boolean): Promise<boolean> {
	...
	try {
		...
		if (resolvedTarget !== stacksRoot && !resolvedTarget.startsWith(stacksRoot + sep)) {
			console.log(`[备份] 实例化堆栈文件: 拒绝操作，目标路径 "${resolvedTarget}" 超出堆栈根目录 "${stacksRoot}"`);
			return false;
		}
