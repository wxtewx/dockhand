...
export class BackupService {
	...
	private async runLocked(
		...
		try {
			...
			try { await this.ports.notify('backup_failed', { title: '备份失败', message: `"${job.targetName}" 的备份任务失败：${message} (${code})`, type: 'error', target: job.targetName, kind: 'backup', errorCode: code }, envId); } catch { /* non‑fatal */ }
