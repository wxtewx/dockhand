		...
		case 'direct': {
			...
			let remoteProjectDir: string | undefined;
			{
				...
				if (plan.stage && plan.projectDir && workingDir) {
					...
					console.log(`[堆栈:${stackName}] 直连环境：已在远程主机 ${plan.projectDir} 暂存 ${staged} 个文件 (${plan.reason})`);
			...
			if (deleteFiles && envId != null) {
				try {
					...
					if (env?.connectionType === 'direct') {
						...
						if (base) console.log(`[堆栈:${stackName}] 远程主机 ${base}/${stackName} 目录下的暂存文件将保留 (不执行删除操作，该目录可能存放用户数据)`);
					}
