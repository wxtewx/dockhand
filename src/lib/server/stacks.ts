...
async function executeLocalCompose(
	...
	try {
		...
		try {
			...
			if (timedOut) {
				return {
					success: false,
					output: stdout,
					error: `docker compose ${operation} 执行超时，超时时间为 ${COMPOSE_TIMEOUT_MS / 1000} 秒`,
					command: commandStr
				};
			}
			...
			if (code === 0) {
				...
				return {
					success: false,
					output: stdout,
					error: stderr || `docker compose ${operation} 已退出，退出码为 ${code}`,
					command: commandStr
				};
			}
		} finally {
		...
		return {
			success: false,
			output: '',
			error: `无法运行 docker compose ${operation}：${err.message}`,
			command: commandStr
		};
	} finally {
