...
async function executeLocalCompose(
	...
	if (dockerHost && operation === 'up') {
		...
		if (relativeBinds.length > 0) {
			const message = `该堆栈使用了相对路径绑定挂载（${relativeBinds.join(', ')}），无法部署至远程 Docker 环境。路径会在 Dockhand 服务端解析，而非目标宿主机。解决方式：将绑定挂载改为命名数据卷，或使用目标主机上已存在的绝对路径。`;
			console.log(`${logPrefix} 拒绝执行: ${message}`);
			return {
				success: false,
				output: '',
				error: message
	...
	if (useOverrideFile && envVars && Object.keys(envVars).length > 0) {
		...
		const header = '# 由 Dockhand 自动生成。请勿手动编辑 - 下次部署时改动将会被覆盖。\n';
	...
	try {
		...
		if (result.success) {
			...
			return {
				...
				error: result.error || `Compose ${operation} 操作执行失败`,
				deletion
			};
		}
	} catch (err: any) {
		...
		return {
			...
			error: isStringLength
				? `堆栈文件过大，无法通过 Hawser 传输。该仓库可能包含大型二进制文件，建议使用 .dockerignore 或将大文件移出编排目录。`
				: `通过 Hawser 执行 ${operation} 操作失败：${err.message}`
		};
	}
}
