			...
			wss.on('connection', (ws: WsWebSocket, req: any) => {
				...
				(async () => {
					...
					if (typeof authFn !== 'function') {
						ws.send(JSON.stringify({ type: 'error', message: '服务暂不可用' }));
					...
					if (typeof canAccessFn === 'function') {
						...
						if (!ok) {
							console.warn(`[WS] 环境访问权限拒绝：用户=${wsAuth.username} 环境 ID=${envId}`);
					...
					if (typeof canExecFn === 'function') {
						...
						if (!allowed) {
							console.warn(`[WS] 执行操作被拒绝: user=${wsAuth.username} envId=${envId}`);
							ws.send(JSON.stringify({ type: 'error', message: '权限被拒绝' }));
				...
				ws.on('error', (err: Error) => {
					console.error('[Terminal WS] 连接异常:', err.message);
				});
			});
